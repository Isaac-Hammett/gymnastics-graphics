...
8. Persist playout rules to Firebase — RundownEditor saves `playout` and `content-sequence` segment config to `competitions/{compId}/rundown/segments/{segId}`, playoutEngine reads rules on activation — NOT STARTED
...
11. Wire moment replay to engine — Producer `flagMoment` with `playNow: true` inserts moment into priority stack after current clip, coordinator writes `moment-replay` type to `currentGraphic` with seekStart/seekEnd/speed, output.html handles video seek and playback rate — NOT STARTED
