...
## Reference: Scene Templates

Production scene templates are stored in `server/config/sceneTemplates/`. These are full OBS scene collection exports that define the standard scene configurations for different competition types.

### Available Templates

| File | Competition Type | Scenes |
|------|------------------|--------|
| `20260119-obs-template-ai-dual.json` | Dual meets (2 teams) | 9 scenes |
| `20260119-obs-template-ai-quad.json` | Quad meets (4 teams) | 22 scenes |
...
|------------|------|---------|
| Stream Starting Soon | Static | Pre-show holding screen |
| Full Screen - Camera A | Single | Full-screen camera A |
| Full Screen - Camera B | Single | Full-screen camera B |
| Dual View - Camera A - Left | Multi | Side-by-side, Camera A on left |
| Dual View - Camera A - Right | Multi | Side-by-side, Camera A on right |
| Web-graphics-only-no-video | Graphics | Graphics overlay only (no video) |
| Replay - Camera A | Replay | Replay from camera A |
| Replay - Camera B | Replay | Replay from camera B |
| End Stream | Static | Sign-off screen |

### Quad Meet Template (22 scenes)

| Type | Scenes |
|------|--------|
| Static | Stream Starting Soon, End Stream |
| Full Screen | Camera A, B, C, D (4 scenes) |
| Dual View | 6 combinations: A&B, A&C, A&D, B&C, B&D, C&D |
| Triple View | 4 combinations: ABC, ABD, ACD, BCD |
| Quad View | 1 scene (all 4 cameras) |
| Replay | Camera A, B, C, D (4 scenes) |
| Graphics | Web-graphics-only-no-video |

### Template Structure

Each template JSON contains:
- `scene_order` - Ordered list of scene names
- `sources` - All source definitions (inputs, scenes with items, transforms)
- `transitions` - Transition effects (Swipe, Stinger)
- `quick_transitions` - Hotkey-accessible transitions
- Audio device configurations

### Naming Conventions

Scene names follow consistent patterns for easy categorization:
- **Static scenes**: "Stream Starting Soon", "End Stream"
- **Single camera**: "Full Screen - Camera X"
- **Multi-camera**: "Dual View - Camera X - Left/Right" or "Dual View - Camera X & Camera Y"
- **Triple/Quad**: "Triple View - Camera X Y Z", "Quad View"
- **Replay**: "Replay - Camera X"
- **Graphics**: "Web-graphics-only-no-video"
