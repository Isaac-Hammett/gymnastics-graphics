#!/bin/bash
# Single-run discovery for PRD-Auth-Login
#
...
# Then run run-Auth-Login.sh for the automated loop.
...
echo "=== Auth-Login Discovery ($(date)) ===" | tee -a "$LOG_FILE"

cat "$SCRIPT_DIR/prompt-Auth-Login-Discovery.md" | \
...
echo ""
echo "Next steps:"
echo "  1. Review updated plan: docs/PRD-Auth-Login/implementation-plan.md"
echo "  2. Manually run first execution iteration:"
echo "     cat $SCRIPT_DIR/prompt-Auth-Login.md | claude -p --dangerously-skip-permissions --mcp-config $MCP_CONFIG"
echo "  3. If that passes, start the loop:"
echo "     bash $SCRIPT_DIR/run-Auth-Login.sh"
