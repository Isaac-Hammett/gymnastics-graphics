#!/bin/bash
# Autonomous implementation loop for PRD-{FeatureName}
#
# Prerequisites (do these manually before running this script):
#   1. Discovery run complete: bash run-{FeatureName}-Discovery.sh
#   2. First iteration verified manually:
#      cat prompt-{FeatureName}.md | claude -p --dangerously-skip-permissions --mcp-config ../.mcp.json
#
...
PRD_FILE="$SCRIPT_DIR/PRD-{FeatureName}-{date}.md"
...
for i in $(seq 1 $MAX_ITERATIONS); do
    ...
    cat "$SCRIPT_DIR/prompt-{FeatureName}.md" | \
