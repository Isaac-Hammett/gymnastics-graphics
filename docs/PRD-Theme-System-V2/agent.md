...
22. **rotation-slate CSS variable conversion (Task 7E.1):** Both `rotation-slate.html` and `rotation-slate-auto.html` now use CSS variables for all overridable properties. Colors use 3-layer cascade (`--rotation-slate-X, --meet-X, fallback`). Font families are in a cross-layout section at the top. Font weights and text-transforms use per-layout var() with original values as defaults. The static version uses `rotation-slate` prefix, auto version uses `rotation-slate-auto` prefix. Theme-loader.js mappings at lines 677-698 include: meetNameFontFamily, meetNameFontWeight, meetNameTextTransform, rotationLabelFontFamily, rotationLabelFontWeight, rotationLabelTextTransform, rotationNumberFontFamily, rotationNumberFontWeight, badgeLabelFontFamily, badgeLabelFontWeight, badgeNumFontFamily, badgeNumFontWeight, statusLabelFontFamily, statusLabelFontWeight, statusLabelFontSize, cinemaBg.

23. **rotation-slate-auto extra CSS:** Has `.status-label` (font-size, font-weight, font-family all overridable) and `.rotation-number.changing` pulse animation. Status label text-transform is also overridable via `--rotation-slate-auto-status-label-text-transform`.
