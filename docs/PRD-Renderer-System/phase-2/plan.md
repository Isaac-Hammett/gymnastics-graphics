...
- LEARNING: `applyTheme()` in stage.html must read colors from `theme.colors` (not `theme` root) and use v3.0 Firebase field names (`headerBar` → `--meet-header-bg`, `contentArea` → `--meet-content-bg`, etc.). Logos come from `theme.logos`, images from `theme.images`. This matches the structure used by `theme-loader.js`.
