...
DEPLOY_VALIDATION_URL="https://commentarygraphic.com"
...
AGENT_FILE="$SCRIPT_DIR/../phase-1/agent.md"
...
echo -e "${GREEN}=== Standard Loop v2 — Phase 3: Scoring Ingestion ===${NC}"
...
2. Upload and extract to production server (3.87.107.201)
3. Upload output.html and overlays/ if they were modified
4. Verify the site loads at https://commentarygraphic.com

ALSO: Restart the coordinator server (server-side changes in this phase):
1. SSH to 44.193.31.120
2. cd /opt/gymnastics-graphics && git pull origin main
3. pm2 delete coordinator
4. GOOGLE_APPLICATION_CREDENTIALS=/opt/gymnastics-graphics/firebase-service-account.json pm2 start server/index.js --name coordinator
5. pm2 save
6. Verify: curl http://44.193.31.120:3003/health
DEPLOY_PROMPT
            if [ ${PIPESTATUS[0]} -ne 0 ]; then
