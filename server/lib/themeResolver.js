/**
 * Theme Resolution Helper (Server-Side)
 *
 * Resolves a theme from Firebase with per-graphic overrides baked in.
 * Used by timesheetEngine for stage engine graphics rendering.
 *
 * This is the server-side version using Firebase Admin SDK.
 * See show-controller/src/lib/themeResolver.js for the client-side version.
 */
 ...
 *
 * @param {Database} db - Firebase Admin SDK database instance
...
async function resolveTheme(db, themeId, graphicId) {
  ...
  if (!db) {
    console.error('[themeResolver] No database instance provided');
    return null;
  }
 ...
 */
function themeToCssVars(resolved) {
  if (!resolved) return {};
