...
// Load graphics registry from JSON (generated by buildGraphicsRegistry.js)
...
try {
  ...
  console.warn('[TimesheetEngine] Could not load graphics registry:', error.message);
  console.warn('[TimesheetEngine] Stage renderer routing will default to "output"');
}
 ...
 */
function getGraphicById(graphicId) {
...
class TimesheetEngine extends EventEmitter {
  ...
  async _triggerGraphic(segment) {
    ...
    if (firebaseRenderer === 'stage' && this.firebase && this.compId && registryEntry) {
      try {
        const db = typeof this.firebase.ref === 'function' ? this.firebase : this.firebase.database();
        ...
        console.log(`[Timesheet${this.compId ? ':' + this.compId : ''}] Stage graphic "${graphicId}" - skeleton: ${registryEntry.skeleton}, blocks: ${registryEntry.blocks?.join(', ') || 'none'}, theme: ${meetTheme || 'none'}`);
      } catch (error) {
        console.warn(`[Timesheet${this.compId ? ':' + this.compId : ''}] Failed to resolve theme for stage graphic: ${error.message}`);
